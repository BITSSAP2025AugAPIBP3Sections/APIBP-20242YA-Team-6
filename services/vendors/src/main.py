...
@app.get("/v1/vendors/search/advanced", response_model=PaginatedVendorResponse)
async def advanced_search_vendors(
    ...
    name_contains: Optional[str] = Query(None, description="Name contains (case-insensitive)"),
    email_domain: Optional[str] = Query(None, description="Email domain filter"),
    ...
    global_search: Optional[str] = Query(None, description="Search across all fields"),
    user=Depends(verify_token),
    db: Session = Depends(get_db)
):
    """
    Advanced search endpoint with complex filtering options.
    """
    query = db.query(DBVendor)
    
    # Advanced filtering
    if name_contains:
        query = query.filter(DBVendor.name.ilike(f"%{name_contains}%"))
    
    if email_domain:
        query = query.filter(DBVendor.email.ilike(f"%@{email_domain}%"))
    
    if event_ids:
        event_id_list = [eid.strip() for eid in event_ids.split(",") if eid.strip()]
        query = query.filter(DBVendor.event_id.in_(event_id_list))
    
    if has_phone is not None:
        if has_phone:
            query = query.filter(and_(DBVendor.phone.isnot(None), DBVendor.phone != ""))
        else:
            query = query.filter(or_(DBVendor.phone.is_(None), DBVendor.phone == ""))
    ...
    applied_filters = {}
    if name_contains: applied_filters["name_contains"] = name_contains
    if email_domain: applied_filters["email_domain"] = email_domain
    if event_ids: applied_filters["event_ids"] = event_ids
    if has_phone is not None: applied_filters["has_phone"] = has_phone
    if global_search: applied_filters["global_search"] = global_search
...
@app.get("/v1/vendors/stats/summary")
async def get_vendor_stats(
    user=Depends(verify_token),
    db: Session = Depends(get_db)
):
    """Get summary statistics about vendors."""
    total_vendors = db.query(DBVendor).count()
    vendors_with_phone = db.query(DBVendor).filter(
        and_(DBVendor.phone.isnot(None), DBVendor.phone != "")
    ).count()
    
    # Get unique event count
    unique_events = db.query(DBVendor.event_id).distinct().count()
    
    return {
        "total_vendors": total_vendors,
        "vendors_with_phone": vendors_with_phone,
        "vendors_without_phone": total_vendors - vendors_with_phone,
        "unique_events": unique_events,
        "phone_coverage_percentage": round((vendors_with_phone / total_vendors * 100) if total_vendors > 0 else 0, 2)
    }
